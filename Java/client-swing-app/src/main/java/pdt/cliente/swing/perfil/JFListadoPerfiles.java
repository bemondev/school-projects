...
/**
 * Clase que representa la ventana de listado de perfiles.
 */
public class JFListadoPerfiles extends javax.swing.JFrame {

    // ======================== Atributos ========================
    /**
     * Instancia única de la clase (patrón Singleton).
     */
    private static JFListadoPerfiles instancia;
     ...
     */
    public static JFListadoPerfiles getInstancia() {
        if (instancia == null) {
            instancia = new JFListadoPerfiles();
        }
        return instancia;
    }
    ...
    /**
     * Rellena una fila de la tabla con los datos de un perfil.<br>
     * Si la fila no existe, crea una nueva y la selecciona.
     *
     * @param perfil Perfil a cargar.
     * @param fila   Fila a cargar.
     */
    public void cargarFila(PerfilDTO perfil, int fila) {
        // Obtiene el modelo de la tabla
        DefaultTableModel modeloTabla = (DefaultTableModel) tblPerfiles.getModel();
        ...
        // Carga los datos del perfil en la fila
        modeloTabla.setValueAt(perfil.getId(), fila, 0);
        modeloTabla.setValueAt(perfil.getNombre(), fila, 1);
        modeloTabla.setValueAt(perfil.getEstado().name(), fila, 2);
    }

    /**
     * Método que recarga las filas de la tabla con los perfiles de la base de datos.
     */
    public void recargarFilas() {
        ...
        // Agrega cada perfil a la tabla
        for (PerfilDTO perfil : perfiles) {
            this.cargarFila(perfil, modeloTabla.getRowCount());
        }
    }
    ...
    public void aplicarFiltros() {
        ...
        this.tblPerfiles.setRowSorter(sorter);
    ...
    private void cargarEventoSeleccion() {
        ListSelectionListener selectionTableListener = e -> {
            ...
            if (tblPerfiles.getSelectedRow() != -1) {
                int filaSeleccionada = tblPerfiles.getSelectedRow();
                int id = (int) tblPerfiles.getModel().getValueAt(tblPerfiles.getRowSorter().convertRowIndexToModel(filaSeleccionada), 0);
                PerfilDTO perfil = Conexion.rec_perfil.obtenerPorID(id);
                if (perfil.getEstado().equals(EstadosEnum.ELIMINADO)) {
        ...
        this.tblPerfiles.getSelectionModel().addListSelectionListener(selectionTableListener);
    }
    // =============================== Eventos ===============================
     ...
     * Obtiene el perfil seleccionado y muestra la ventana de ficha de perfil.
     * @param evt Evento de acción.
     */
    private void btnModificarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnModificarActionPerformed
        int filaSeleccionada = this.tblPerfiles.getSelectedRow();
        if (filaSeleccionada == -1) {
            JOptionPane.showMessageDialog(this, "Debe seleccionar un perfil.");
            return;
        }
        int id = (int) this.tblPerfiles.getModel().getValueAt(this.tblPerfiles.getRowSorter().convertRowIndexToModel(filaSeleccionada), 0);

        PerfilDTO perfildto = Conexion.rec_perfil.obtenerPorID(id);
        JFFichaPerfil ficha = new JFFichaPerfil(perfildto);
    ...
    private void btnBajaReactivarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnBajaReactivarActionPerformed
        ...
        if (perfildto.getEstado() == EstadosEnum.ACTIVO) {
            Conexion.rec_perfil.desactivar(perfildto.getId());
            perfildto.setEstado(EstadosEnum.ELIMINADO);
            btnBajaReactivar.setText("Reactivar");
            JOptionPane.showMessageDialog(this, "Perfil " + perfildto.getNombre() + " desactivado.");
        } else if (perfildto.getEstado() == EstadosEnum.ELIMINADO) {
            Conexion.rec_perfil.activar(perfildto.getId());
            perfildto.setEstado(EstadosEnum.ACTIVO);
            btnBajaReactivar.setText("Reactivar");
            JOptionPane.showMessageDialog(this, "Perfil " + perfildto.getNombre() + " activado.");
        }

        this.cargarFila(perfildto, filaSeleccionada);
    ...
    private void btnExportarExcelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnExportarExcelActionPerformed
        try {
            ExcelManager.exportarExcel(tblPerfiles);
    ...
    private void initComponents() {
        ...
        tblPerfiles.setAutoCreateRowSorter(true);
        tblPerfiles.setBackground(new java.awt.Color(217, 217, 217));
        ...
        tblPerfiles.setColumnSelectionAllowed(true);
        tblPerfiles.setMinimumSize(new java.awt.Dimension(1007, 558));
        tblPerfiles.setPreferredSize(new java.awt.Dimension(1007, 558));
        scrollPanelTabla.setViewportView(tblPerfiles);
        tblPerfiles.getColumnModel().getSelectionModel().setSelectionMode(javax.swing.ListSelectionModel.SINGLE_SELECTION);

        lblListadoDeUsuarios.setFont(new java.awt.Font("Unispace", 0, 36)); // NOI18N
        lblListadoDeUsuarios.setText("Listado de Perfiles");
