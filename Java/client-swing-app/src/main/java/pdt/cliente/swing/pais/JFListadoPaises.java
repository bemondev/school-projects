package pdt.cliente.swing.pais;
...
/**
 * Clase que representa la ventana de listado de países.
 */
public class JFListadoPaises extends javax.swing.JFrame {

    // ======================== Atributos ========================
    /**
     * Instancia única de la clase (patrón Singleton).
     */
    private static JFListadoPaises instancia = null;
     ...
     */
    public static JFListadoPaises getInstancia() {
        if (instancia == null) {
            instancia = new JFListadoPaises();
        }
        return instancia;
    }
    ...
    /**
     * Rellena una fila de la tabla con los datos de un país.<br>
     * Si la fila no existe, crea una nueva y la selecciona.
     *
     * @param pais País a cargar.
     * @param fila Fila a cargar.
     */
    public void cargarFila(PaisDTO pais, int fila) {
        // Obtiene el modelo de la tabla
        DefaultTableModel modeloTabla = (DefaultTableModel) tblPaises.getModel();
        ...
        // Carga los datos del país en la fila
        modeloTabla.setValueAt(pais.getId(), fila, 0);
        modeloTabla.setValueAt(pais.getNombre(), fila, 1);
        modeloTabla.setValueAt(pais.getEstado().name(), fila, 2);
    }

    /**
     * Método que recarga las filas de la tabla con los países de la base de datos.
     */
    public void recargarFilas() {
        ...
        // Agrega cada país a la tabla
        for (PaisDTO pais : paises) {
            this.cargarFila(pais, modeloTabla.getRowCount());
        }
    }
    ...
    public void aplicarFiltros() {
        ...
        this.tblPaises.setRowSorter(sorter);
    ...
    private void cargarEventoSeleccion() {
        ListSelectionListener selectionTableListener = e -> {
            ...
            if (tblPaises.getSelectedRow() != -1) {
                int filaSeleccionada = tblPaises.getSelectedRow();
                int id = (int) tblPaises.getModel().getValueAt(tblPaises.getRowSorter().convertRowIndexToModel(filaSeleccionada), 0);
                PaisDTO pais = Conexion.rec_pais.obtenerPorID(id);
                if (pais.getEstado().equals(EstadosEnum.ELIMINADO)) {
        ...
        this.tblPaises.getSelectionModel().addListSelectionListener(selectionTableListener);
    }
     ...
     * Realiza la baja o activación de un país.
     *
     * @param evt Evento de acción.
     */
    private void btnBajaReactivarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnBajaReactivarActionPerformed
        int filaSeleccionada = this.tblPaises.getSelectedRow();
        if (filaSeleccionada == -1) {
            JOptionPane.showMessageDialog(this, "Debe seleccionar un País.");
            return;
        }
        int id = (int) this.tblPaises.getModel().getValueAt(this.tblPaises.getRowSorter().convertRowIndexToModel(filaSeleccionada), 0);
        ...
        if (respuesta == JOptionPane.YES_OPTION) {
            PaisDTO dto = Conexion.rec_pais.obtenerPorID(id);
            if (dto.getEstado() == EstadosEnum.ACTIVO) {
                Conexion.rec_pais.desactivar(dto.getId());
                dto.setEstado(EstadosEnum.ELIMINADO);
                JOptionPane.showMessageDialog(this, "País " + dto.getNombre() + " desactivado.");
            } else if (dto.getEstado() == EstadosEnum.ELIMINADO) {
                Conexion.rec_pais.activar(dto.getId());
                dto.setEstado(EstadosEnum.ACTIVO);
                JOptionPane.showMessageDialog(this, "País " + dto.getNombre() + " activado.");
            }
            recargarFilas();
            aplicarFiltros();
        }
    ...
    private void btnExportarExcelActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnExportarExcelActionPerformed
        try {
            ExcelManager.exportarExcel(tblPaises);
    ...
    private void initComponents() {
        ...
        tblPaises.setAutoCreateRowSorter(true);
        tblPaises.setBackground(new java.awt.Color(217, 217, 217));
        ...
        tblPaises.setColumnSelectionAllowed(true);
        tblPaises.setMinimumSize(new java.awt.Dimension(1007, 558));
        tblPaises.setPreferredSize(new java.awt.Dimension(1007, 558));
        scrollPanelTabla.setViewportView(tblPaises);
        tblPaises.getColumnModel().getSelectionModel().setSelectionMode(javax.swing.ListSelectionModel.SINGLE_SELECTION);

        lblListadoDeUsuarios.setFont(new java.awt.Font("Unispace", 0, 36)); // NOI18N
        lblListadoDeUsuarios.setText("Listado de Paises");
        ...
        btnBajaReactivar.setText("Bajar / Activar");
