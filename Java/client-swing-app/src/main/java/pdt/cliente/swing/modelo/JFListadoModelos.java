...
/**
 * Clase que representa la ventana de listado de modelos.
 */
public class JFListadoModelos extends javax.swing.JFrame {

    // ======================== Atributos ========================
    /**
     * Instancia única de la clase (patrón Singleton).
     */
    private static JFListadoModelos instancia;
     ...
     */
    public static JFListadoModelos getInstancia() {
        if (instancia == null) {
            instancia = new JFListadoModelos();
        }
        return instancia;
    }
    ...
    /**
     * Rellena una fila de la tabla con los datos de un modelo.<br>
     * Si la fila no existe, crea una nueva y la selecciona.
     *
     * @param modelo Modelo a cargar.
     * @param fila   Fila a cargar.
     */
    public void cargarFila(ModeloDTO modelo, int fila) {
        // Obtiene el modelo de la tabla
        DefaultTableModel modeloTabla = (DefaultTableModel) tblModelos.getModel();
        ...
        // Carga los datos del modelo en la fila
        modeloTabla.setValueAt(modelo.getId(), fila, 0);
        modeloTabla.setValueAt(modelo.getNombre(), fila, 1);
        modeloTabla.setValueAt(modelo.getEstado().name(), fila, 2);
    }
    ...
    public void aplicarFiltros() {
        ...
        this.tblModelos.setRowSorter(sorter);
    ...
    /**
     * Método que recarga las filas de la tabla con los modelos de la base de datos.
     */
    public void recargarFilas() {
        ...
        // Obtiene la lista completa de modelos
        List<ModeloDTO> modelos = Conexion.rec_modelo.obtenerTodo();

        // Agrega cada modelo a la tabla
        for (ModeloDTO modelo : modelos) {
            this.cargarFila(modelo, modeloTabla.getRowCount());
        }
    }
    ...
    private void cargarEventoSeleccion() {
        ListSelectionListener selectionTableListener = e -> {
            ...
            if (tblModelos.getSelectedRow() != -1) {
                int filaSeleccionada = tblModelos.getSelectedRow();
                int id = (int) tblModelos.getModel().getValueAt(tblModelos.getRowSorter().convertRowIndexToModel(filaSeleccionada), 0);
                ModeloDTO modelo = Conexion.rec_modelo.obtenerPorID(id);
                if (modelo.getEstado().equals(EstadosEnum.ELIMINADO)) {
        ...
        this.tblModelos.getSelectionModel().addListSelectionListener(selectionTableListener);
    }
    ...
    /**
     * Método que se ejecuta al presionar el botón "Baja / Activar".<br>
     * Realiza la baja o activación de un modelo.
     *
     * @param evt Evento de acción.
     */
    private void btnBajaReactivarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnBajaReactivarActionPerformed
        int filaSeleccionada = this.tblModelos.getSelectedRow();
        if (filaSeleccionada == -1) {
            JOptionPane.showMessageDialog(this, "Debe seleccionar un modelo.");
            return;
        }
        int id = (int) this.tblModelos.getModel().getValueAt(this.tblModelos.getRowSorter().convertRowIndexToModel(filaSeleccionada), 0);

        ModeloDTO dto = Conexion.rec_modelo.obtenerPorID(id);
        ...
        if (respuesta == JOptionPane.YES_OPTION) {
            if (dto.getEstado() == EstadosEnum.ACTIVO) {
                Conexion.rec_modelo.desactivar(dto.getId());
                dto.setEstado(EstadosEnum.ELIMINADO);
                JOptionPane.showMessageDialog(this, "Modelo: " + dto.getNombre() + " desactivado.");

            } else if (dto.getEstado() == EstadosEnum.ELIMINADO) {
                Conexion.rec_modelo.activar(dto.getId());
                dto.setEstado(EstadosEnum.ACTIVO);
                JOptionPane.showMessageDialog(this, "Modelo: " + dto.getNombre() + " activado.");
            }
    ...
    private void btnExportarExcelActionPerformed(java.awt.event.ActionEvent evt) {                                                 
        try {
            ExcelManager.exportarExcel(tblModelos);
    ...
    private void initComponents() {
        ...
        tblModelos.setAutoCreateRowSorter(true);
        tblModelos.setBackground(new java.awt.Color(217, 217, 217));
        ...
        tblModelos.setColumnSelectionAllowed(true);
        tblModelos.setMinimumSize(new java.awt.Dimension(1007, 558));
        tblModelos.setPreferredSize(new java.awt.Dimension(1007, 558));
        scrollPanelTabla.setViewportView(tblModelos);
        tblModelos.getColumnModel().getSelectionModel().setSelectionMode(javax.swing.ListSelectionModel.SINGLE_SELECTION);
        ...
        lblListadoDeUsuarios.setText("Listado de Modelos");
